...
Возвращается в ответ на вызовы методов группы [`/messages`](../methods/GET/messages/index.md) и [`/chats/{chatId}/pin`](../methods/PUT/chats/-chatId-/pin.md)
    ...
    - **`attachments`** — Attachment[] · Nullable
      ...
      - Вариант `type = "video"` — VideoAttachment
        ...
        - **`payload`** — object MediaAttachmentPayload
          ...
          - **`url`** — string
            ...
            URL вложения, который возвращается в объекте [Update](Update.md) после отправки сообщения в чат или канал

            В случае с вложенным видео прямую ссылку также можно получить с помощью метода [`GET /videos/{-videoToken-}`](../methods/GET/videos/-videoToken-.md)
      ...
      - Вариант `type = "contact"` — ContactAttachment
        ...
        - **`payload`** — object ContactAttachmentPayload
          ...
          - **`max_info`** — object [User](User.md) · Nullable · optional
