...
- `chat_id` — метод возвращает массив сообщений или постов из указанного чата или канала, только если чат-бот в них является администратором. Сообщения возвращаются в обратном порядке: последние сообщения будут первыми в массиве. Как получить ID — в [разделе «Получение chat_id»](../../../index.md#Получение%20chat_id)
...
- **`chat_id`** — integer bigint &lt;int64&gt; · optional

  ID чата или канала, чтобы получить из него сообщения или посты. Обязательный параметр, если не указан `message_ids`. Как получить ID — в [разделе «Получение chat_id»](../../../index.md#Получение%20chat_id)
...
- **`from`** — integer bigint &lt;int64&gt; · optional
...
- **`to`** — integer bigint &lt;int64&gt; · optional
...
- **`messages`** — [Message](../../../objects/Message.md)[]
