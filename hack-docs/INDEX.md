...
**Точка входа для людей и агентов.** Если контекст потерян: `/CLAUDE.md` → этот файл → [MINDMAP](research/MINDMAP.md) → [HANDOFF](HANDOFF.md) (что делается сейчас и следующий шаг) → [GEN_V2_PLAN](GEN_V2_PLAN.md) → заметка по задаче. Правила проекта — в [`/CLAUDE.md`](../CLAUDE.md).
...
## Текущее состояние (24.09.2026, вечер)
- **Этап:** MVP собран. Ядро, все Should и материалы к сдаче готовы в коде и в `docs/`. Идёт сессия по [GEN_V2_PLAN](GEN_V2_PLAN.md): шлифовка, затем новые фичи (подтверждение ремонта, телефон для мастера, кабинет района, импорт и геокодер, председатель совета). Текущий шаг — в [HANDOFF](HANDOFF.md).
- **Фокус (принят):** ядро H1 + H2 + шеринг из H5; пилот Москва; B2G → SaaS ([ADR-002](adr/002-product-scope.md)).
- **Блокеры:** деплой на VPS и проверка в MAX за DevOps; домен стенда и название продукта не выбраны.
- **Вехи:** заморозка кода 29.09 днём, сдача 30.09 ([GEN_V2_PLAN](GEN_V2_PLAN.md)).
...
| [GEN_V2_PLAN.md](GEN_V2_PLAN.md) | v2: шлифовка и новые фичи 24–30.09 по итогам брейншторма, линия отсечения | accepted | 2026-09-24 |
...
| [HANDOFF.md](HANDOFF.md) | заметка передачи: кто в работе, что сделано, следующий шаг | active | 2026-09-24 |
...
| Файл | Когда |
|---|---|
| `adr/016` геокодер и лицензии открытых данных, `adr/017` роли председателя совета и района | части 6–7 [GEN_V2_PLAN](GEN_V2_PLAN.md) |
