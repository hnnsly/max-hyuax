...
## Сейчас (25.09, ночь) — спайк «мини-приложение, живая карточка, диалог бота»
- **Сделано:**
  ...
  - `miniapp/` (Vite 8 + React 19.2.8 + TS 7 strict + `@maxhub/max-ui` 0.5.0 + fontsource Sofia Sans ×3 + phosphor + Vitest):
    ...
    - роутер-стек с нативной `BackButton`; форма заявки пока заглушка.
...
- **Не проверено руками в MAX:** живая карточка и диалог. Проверка: `BOT_MODE=polling` в `.env` → `task run` → написать боту → из демо УК сменить статус.
- **Дальше:** форма заявки из 3 шагов и дубль, очередь УК со сменой статуса (часть D плана), затем 27.09.
