...
## 7. To-Be в коде (26.09)
| Шаг To-Be из §2 | Состояние |
|---|---|
| QR в лифте или команда боту | есть: наклейки с QR (`startapp=o_<код>`, PDF A6), заявка боту фразой или кнопками, геопозиция |
| Объект, ответственный и срок определены | есть: справочник `rules` с производственным календарём, подсказка категории (правила + Ollama) |
| «Это и у меня» | есть: форма, карточка, бот |
| Новая заявка за 3 шага с фото | есть: форма в 3 шага, фото без EXIF в MinIO, черновик |
...
| УК видит масштаб и меняет статус | есть: очередь по срочности с фильтрами, метрики, фото-пруф, карта нагрузки домов (ADR-020); в боте очередь, смена статуса и метрики (ADR-019) |
| PDF для эскалации в ГЖИ | есть: черновик обращения по подписанной ссылке, PDF-реестр дома |
| Роли председателя совета и района, проверка ремонта жителями | есть: совет дома и опросы, кабинет района с картой района, «Починили» и «Не починили» (ADR-017, ADR-020); в боте меню по роли |
| `requestContact` из цепочки бонуса (§5) | есть: в мини-приложении и кнопкой `request_contact` в боте |
| Бот как полноценный канал | есть: одно окно, экраны заменяют сообщение (ADR-019) |
| Голос и доступность для 60+ и незрячих | есть: заявка голосом по расшифровке MAX, режим «Крупно и контрастно», экранный чтец (ADR-021) |
| Рейтинг УК района для жителей | есть: оценка ремонта 1–5, рейтинг 0–100 в приложении и боте (ADR-022) |
| Коллективное обращение | в работе по [GEN_V3_PLAN](../GEN_V3_PLAN.md) |
