...
14. **Совет дома:** `POST /proposals`, `GET /me/proposals`, `GET /polls`, `POST /polls/{id}/vote`; председатель: `GET /council/proposals`, `POST /council/proposals/{id}/reply`, `POST /council/polls`. Коды `already_voted`, `poll_closed` и `poll_exists` (по предложению опрос уже открыт) приходят с 409.
