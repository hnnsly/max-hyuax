...
Сервисом пользуются жители 60+ и люди со слабым зрением.

**Режим «Крупно и контрастно»** включается переключателем в блоке «Удобство» внизу «Моего дома» и кабинетов. Для проверки есть ссылка `?a11y=large`.
- **Шрифты:** текст крупнее (основной 21 px, подписи не меньше 17 px), шрифты MAX UI увеличиваются на четверть.
- **Контраст:** вторичный текст темнее, контраст не ниже 7:1. Системная настройка «повышенный контраст» включает те же цвета и без режима.
- **Кнопки:** крупнее, текст на них не обрезается.
- **Хранение:** настройка хранится на устройстве.

**Экранный чтец** (TalkBack, VoiceOver):
- **Заголовок экрана:** он есть всегда, в MAX скрыт визуально, потому что шапку рисует клиент. При переходе на новый экран фокус ставится на него, меняется и заголовок вкладки.
- **Строка заявки** читается по смыслу: что сломалось, где, сколько соседей сообщили, срок. Статус читается как «Статус: …».
- **Тосты** объявляются через постоянную область `role="status"`.
- **Карта:** точки — кнопки с подписью дома и числа заявок, кнопки масштаба подписаны по-русски. Рядом всегда есть вид «Списком».
- **Клавиатура и пульты:** ячейки меню нажимаются клавишами Enter и пробел.

**Голосом в боте.** Заявку можно наговорить: MAX присылает расшифровку голосового, бот работает с ней как с текстом (раздел «Бот»).
...
- **Коллективное обращение в жилинспекцию (ADR-023).** У просроченной открытой заявки её участники видят кнопку «Подписать обращение в ГЖИ» и «Скачать PDF».
  - Соседи по дому могут официально поддержать обращение своими подписями (`POST /issues/{id}/appeal/sign`), опционально указав ФИО и квартиру. Подпись можно отозвать в любой момент (`DELETE /issues/{id}/appeal/sign`).
  - По ссылке `GET /appeal/{token}` сервер генерирует официальный PDF: адресат (Мосжилинспекция), адрес дома, УК, дата подачи, срок и его нормативное основание (ПП № 416), общее число поддержавших соседей, официальная таблица заявителей с датами и номерами квартир, хронология без имён и строки для личных подписей.
  - Заголовок авторизации ссылке не нужен: в MAX файл скачивается через `WebApp.downloadFile`, который заголовков не передаёт. В браузере работает обычная ссылка с `download`, а при сбое скачивания на iOS PWA доступен шеринг ссылки через `bridge.share`.
  - PDF собирается библиотекой `signintech/gopdf` (MIT) со шрифтом Go (BSD). Шрифт встроен в бинарник, с поддержкой кириллицы.
- **Очередь (outbox).**
...
## Оценка ремонта и рейтинг УК района

- **Оценка.** После «Починили» житель оценивает ремонт от 1 до 5 звёзд, один раз, в те же 7 дней.
  - **Мини-приложение:** звёзды появляются в карточке сразу после «Починили».
  - **Бот:** кнопки 1–5 приходят в ответ на «Починили», они же есть в карточке заявки.

  Оценка хранится в строке ответа (`issue_confirmations.stars`), повторная оценка отклоняется (`409 already_rated`).
- **Средняя оценка** ремонтов за 30 дней видна в метриках УК и в кабинете района.
- **Рейтинг УК района** (`GET /district/rating`) видят:
  - житель — по району своего дома, его УК отмечена;
...
- **Итоговая оценка от 0 до 100:**
  ...
  | Доля | Что считается |
  |---|---|
  | 50% | заявки, закрытые в срок |
  | 30% | средняя оценка ремонтов жителями, делённая на 5 |
  | 20% | ремонты, которые жители подтвердили |

  Если оценок нет, их вес делится между сроком и подтверждениями. При меньше чем 3 закрытых за период заявках УК стоит в конце списка с пометкой «мало данных». Имена оценивших нигде не показываются.
- **Где смотреть:**
  - мини-приложение: «Рейтинг УК района» на «Моём доме»;
  - бот: пункт меню жителя и команда `/rating`.
...
- **Где.**
  - Вкладка «Карта» в кабинете УК: дома своей УК.
  - Кнопка «Карта района» в кабинете района: дома района.
  - Жителю карта нагрузки не показывается.
- **Данные** (`GET /map/houses`). По каждому дому с координатами:
  - открытые заявки;
  - просроченные заявки (открыта и срок прошёл, как в метриках района);
  - сколько дней просрочена самая давняя.
- **Точка дома:**
  - площадь растёт с числом открытых заявок, цифра на точке — сколько открыто;
  - цвет: красный — есть просрочка, синий — есть открытые, серый — заявок нет;
  - просроченные дома лежат поверх остальных.

  Нажатие открывает лист дома с открытыми заявками, из него — карточка заявки.
- **Карта.**
  - Библиотека MapLibre GL грузится отдельным файлом только на экранах карты.
  - Подложка OpenFreeMap без ключей: светлая `positron` и тёмная `dark` под тему приложения. Атрибуцию OpenMapTiles и OpenStreetMap показывает сама карта.
  - Точки — обычные кнопки поверх карты: шрифт и цвета приложения, у каждой подпись для экранного чтеца.
- **Запасные пути:**
  - кнопка «Списком» даёт те же дома списком, самые проблемные сверху;
  - без WebGL список открывается сразу;
  - если подложка не загрузилась за 5 секунд, точки остаются на однотонном фоне с пояснением.
- **Выбор дома на карте** (для жителя, экран «Дом на карте» из поиска дома). Работает в MAX, где геолокации нет:
  1. житель приближает карту к своему дому (первое нажатие приближает к точке);
  2. нажимает на здание;
  3. сервис ищет дома рядом с точкой так же, как поиск рядом (`GET /houses/nearest`); дом из OpenStreetMap при необходимости создаётся;
  4. в листе «Какой дом ваш?» житель выбирает дом.
...
- **Опросы.** Председатель открывает опрос по предложению соседа или напрямую по любой теме дома: вопрос и от 2 до 5 вариантов, опрос идёт 7 дней.
...
- **Сводный PDF-отчёт по дому** (`GET /houses/{id}/report.pdf`). На главном экране дома и в папке председателя можно скачать PDF-отчёт по реестру заявок дома (паспорт дома, УК, сводная статистика и полный список заявок со статусами и сроками).
- **Роли.** Председатель — житель с отметкой `users.chairman_house_id`. Для быстрой проверки ролей внутри мессенджера MAX в боте доступна команда `/role` (`chairman`, `uk`, `resident`). УК и управа района в предложениях и опросах не участвуют.
...
Webhook проверяет секрет из заголовка `X-Max-Bot-Api-Secret`, отвечает 200 сразу и обрабатывает событие в фоне. MAX может доставить событие повторно, поэтому ключ каждого события сохраняется в таблице `processed_updates` и повтор пропускается. Бот отвечает только в личных диалогах и не пишет в групповые чаты.
...
- `/role` — роль для проверки на демо-стенде;
- `/help` — справка.

**Одно окно.** Кнопки навигации (меню, списки, карточка заявки, совет дома, «Назад») не присылают новое сообщение, а меняют то, в котором нажаты: бот работает как экраны приложения, и меню не приходится искать, листая чат вверх. Экраны собраны в `internal/transport/bot/screens.go`, данные кнопки — `w:<экран>:<аргументы>`. На каждом экране есть «Меню», на вложенных ещё «Назад». Новое сообщение бот присылает:
- в ответ на текст жителя;
- по команде;
- в уведомлениях.

У таких сообщений в конце тоже есть «Меню». В уведомлениях и живой карточке «Меню» и «Подробнее» присылают экран новым сообщением: само уведомление остаётся в чате, а живую карточку бот продолжает обновлять.

**Меню по роли:**

| Роль | Пункты |
|---|---|
| Житель | «Сообщить о проблеме», «Мои заявки», «Сейчас в доме», «Совет дома», «Мой дом», «Открыть приложение» |
| Председатель совета | пункты жителя и «Папка предложений»; в «Совете дома» ещё «Создать опрос» |
| Сотрудник УК | «Очередь заявок» (открытые и просроченные, сначала срочные), «Метрики» за 30 дней, «Открыть кабинет УК» |
| Управа района | «Сводка района» (УК района: открыто, просрочено, закрыто в срок), «Просрочено в районе», «Открыть кабинет района» |

Житель без дома видит вместо меню «Показать дома рядом» (кнопка геопозиции) и «Найти дом по адресу». На демо-стенде в меню есть «Роль для проверки».

**Заявка кнопками** («Сообщить о проблеме»):
1. **Категория** — кнопки по две в ряд из справочника правил.
2. **Место.** Если у дома несколько объектов этой категории (лифт, свет в каждом подъезде), бот спрашивает подъезд; иначе шаг пропускается.
3. **Описание.** Бот показывает, кто отвечает и до какого срока, и ждёт описание следующим сообщением. Можно нажать «Отправить без описания». Если в доме уже есть открытая заявка этой категории, сверху кнопка «Это и у меня».
4. **Согласие.** Перед первой заявкой бот запрашивает согласие на обработку данных.

**Голосовые сообщения.** MAX присылает расшифровку голосового прямо во вложении (`transcription`). Бот показывает жителю «Текст голосового: «…»» и дальше обрабатывает его как обычный текст: новая проблема, описание заявки, собранной кнопками, комментарий или ответ на вопрос бота. Своего распознавания речи в сервисе нет, аудио не скачивается и не хранится. Если расшифровки нет, бот просит написать текстом.

**Заявка текстом.** Можно просто написать, что сломалось:
1. **Дом.** Если дом не выбран, бот просит геопозицию и показывает ближайшие дома кнопками или предлагает найти дом по адресу в приложении.
2. **Категория.** Бот подсказывает её через модель, если она подключена, иначе по ключевым словам (раздел «Подсказка категории»).
3. **Похожая заявка.** Если в доме уже есть открытая заявка той же категории, бот предлагает «Это и у меня».
4. **Подтверждение.** Кто отвечает и до какого срока, кнопки «Отправить» и «Изменить».
5. **Место.** Подъезд, как в заявке кнопками. Если по этому месту заявка уже есть, житель присоединяется к ней.
6. **Форма.** Длинный текст или неопределённая категория открывают форму мини-приложения.

**Карточка заявки в чате.** «Мои заявки», «Сейчас в доме» и очередь УК открывают карточку на месте: номер, место, статус, ответственный, срок, число сообщивших, комментарий УК и три последних события хронологии. Кнопки зависят от роли и состояния:
- «Это и у меня» — соседу, который ещё не участник;
- «Починили» и «Не починили» — участнику выполненной заявки в течение 7 дней;
- «Оставить телефон для мастера» — участнику открытой заявки, если номер ещё не оставлен;
- «Принять», «В работу», «Выполнено», «Отклонить» — сотруднику ответственной УК, только допустимые из текущего статуса;
- «Открыть в приложении» и «Поделиться».

**Смена статуса в чате.** «Принять» и «В работу» меняют статус сразу, карточка обновляется на месте. «Выполнено» спрашивает комментарий для жителей (можно «Без комментария»), «Отклонить» требует причину. Права и допустимость перехода проверяются до вопроса, так что житель и сотрудник чужой УК получают отказ сразу. Жители узнают о смене статуса из уведомлений, как при смене в кабинете.

**Бот ждёт ответ.** Часть действий требует текста следующим сообщением. Бот запоминает такое действие в таблице `bot_pending` на 10 минут:
- описание заявки, собранной кнопками;
- комментарий сотрудника УК к «Выполнено» или причина отказа;
- комментарий к «Не починили»;
- текст предложения совету и вопрос нового опроса;
- ответ председателя при отказе;
- описание проблемы до выбора подъезда.

Следующее сообщение уходит в это действие, а не считается новой проблемой. Переход на любой экран или команда отменяют ожидание: после «Назад» текст снова считается проблемой. Остальное состояние живёт в данных кнопок, поэтому бот переживает перезапуск.

**Телефон для мастера в боте.** Кнопка типа `request_contact` присылает контакт из аккаунта MAX. Сервер проверяет подпись: `hash` должен совпасть с HMAC-SHA256 от `vcf_info`, где ключ — токен бота. Номер берётся из строки `TEL`. Пересланный контакт без подписи не принимается.

**Совет дома в боте** (`/polls`, «Совет дома» в меню):
- **опросы:** открытые опросы дома видны кнопками, опрос открывается с вариантами, после голоса экран сменяется итогами в процентах;
- **предложения:** «Предложить совету» ждёт текст предложения, «Мои предложения» показывают ответы председателя;
- **председатель:** «Папка предложений» открывает новые предложения по одному с кнопками «Взять в работу» и «Отклонить» (отказ ждёт ответ текстом); «Создать опрос» ждёт вопрос и открывает опрос на 7 дней с вариантами «За», «Против», «Воздержался».

**Уведомления** идут через очередь `outbox` с лимитами Bot API и адресуются только участникам (требования MAX о рассылках):
- **живая карточка:** обновляется у каждого участника при любом изменении заявки;
- **отдельные сообщения** (правка карточки телефон не подсвечивает):
  - «принята» и «в работе»;
  - итог при закрытии (у выполненной заявки кнопки «Починили» и «Не починили»);
  - истечение срока;
  - возврат заявки в работу соседом;
- **совет дома:** председателю приходит новое предложение по дому, автору — ответ председателя. В очереди такие строки ссылаются на предложение (`outbox.proposal_id`), а не на заявку.
