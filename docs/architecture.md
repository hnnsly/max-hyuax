...
- **Где добавить.** В форме новой заявки (до трёх снимков), в карточке заявки и в чате с ботом: присланное фото прикрепляется к последней открытой заявке жителя.
...
- **Обработка на сервере:**
  - принимаются только JPEG и PNG до 5 МБ, не больше 6 фото на заявку;
  - снимок поворачивается по EXIF, уменьшается до 1600 px по длинной стороне и перекодируется в JPEG;
  - EXIF, а с ним геометка и данные камеры, в сохранённый файл не попадает.
- **Хранение.** Файлы лежат на диске в каталоге `PHOTOS_DIR` (в Docker — том `photos`), метаданные в таблице `issue_photos`. Файл отдаётся только с токеном сессии, поэтому мини-приложение загружает его через `fetch`.
- **В боте.** Ссылку на присланное фото MAX передаёт не во всех клиентах. Если ссылки нет, бот предлагает добавить фото в карточке заявки.
