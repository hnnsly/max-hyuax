...
- `/role` — смена роли на демо-стенде;
- `/help` — справка.

**Меню** (после `/start`, `/menu` и выбора дома): «Сообщить о проблеме», «Мои заявки», «Сейчас в доме», «Совет дома», «Мой дом», «Открыть приложение».

**Заявка в диалоге:**
1. **Дом.** Если дом не выбран, бот просит геопозицию («Показать дома рядом») и показывает ближайшие дома кнопками или предлагает найти дом по адресу в приложении.
...
4. **Подтверждение.** Бот показывает, как понял проблему, кто отвечает и до какого срока, с кнопками «Отправить» и «Изменить».
5. **Место.** Если у дома несколько объектов этой категории (лифт, свет в каждом подъезде), бот спрашивает подъезд. Если по этому месту заявка уже есть, житель присоединяется к ней.
6. **Согласие.** Перед первой заявкой бот запрашивает согласие на обработку данных.
7. **Форма.** Длинный текст или неопределённая категория открывают форму мини-приложения.

**Заявка в чате.** «Мои заявки» и «Сейчас в доме» открывают карточку прямо в чате: номер, место, статус, ответственный, срок, число сообщивших, комментарий УК и три последних события хронологии. Кнопки зависят от состояния:
...
- текст предложения совету;
- ответ председателя при отказе;
- описание проблемы до выбора подъезда.

Следующее сообщение жителя уходит в это действие, а не считается новой проблемой. Остальное состояние живёт в данных кнопок, поэтому бот переживает перезапуск.
...
- **опросы:** открытые опросы дома приходят с кнопками вариантов, после голоса сообщение сменяется итогами в процентах;
- **предложения:** «Предложить совету» ждёт текст предложения, «Мои предложения» показывают ответы председателя;
- **председатель:** «Папка предложений» даёт новые предложения с кнопками «Взять в работу» и «Отклонить» (отказ ждёт ответ текстом); председатель может создать опрос.
