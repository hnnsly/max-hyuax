...
- **Кто и когда.** Отметку «выполнено» ставит УК, а проверяют её участники заявки: в течение 7 дней каждый может один раз ответить «Починили» или «Не починили».
- **Где ответить:**
...
- **Кто видит.** Только сотрудник ответственной УК, в блоке «Контакты жителей» карточки. Соседи, чужая УК и списки заявок номер не получают. В ответах о себе (`/me`) номер тоже не отдаётся, только признак `phone_shared`.
- **Согласие.** Оставленный номер и есть согласие показывать его УК. «Не показывать телефон» (`DELETE /me/phone`) стирает номер, удаление аккаунта тоже.
...
- **Хранение.** Файлы лежат на диске в каталоге `PHOTOS_DIR` (в Docker — том `photos`), метаданные в таблице `issue_photos`. Файл отдаётся только с токеном сессии и без кэша, поэтому мини-приложение загружает его через `fetch`.
