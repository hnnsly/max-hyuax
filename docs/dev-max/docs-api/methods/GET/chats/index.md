...
2. **Получите `chat_id` в событии** – о том, как это сделать, см. в [разделе «Получение chat_id»](../../../index.md#Получение%20chat_id)
3. **Сохраните `chat_id`**:
...
4. **Используйте сохранённый `chat_id` для вызовов API**. После того как `chat_id` окажется в вашем хранилище, вы сможете использовать его во всех методах, где он требуется:
   ...
   - [POST `/messages`](../../POST/messages/index.md) — отправить сообщение в чат
   - [GET `/chats/{chat_id}/members`](-chatId-/members/index.md) — получить участников группового чата или канала
...
> Использование [Long Polling](../../../index.md#Long%20Polling) для получения списка чатов и каналов для бота не предусмотрено
