...
- [Библиотека React-компонентов MAX UI](../../ui/index.md), с которой мини-приложение легко стилизовать под интерфейс MAX
...
1. Бот отправляет контент пользователю через [`POST /messages`](../../docs-api/methods/POST/messages/index.md) — например, медиафайл или открытку
...
Подробнее о параметрах — в описании метода [`shareMaxContent()`](bridge.md#Шеринг%20контента)
...
1. Вы формируете диплинк вида `https://max.ru/<botName>?startapp=<payload>` для перехода в мини-приложение, передав в `payload` необходимые стартовые параметры. Убедитесь, что длина `payload` не превышает 512 символов и использованы [допустимые символы](#Payload%20в%20приложении)
...
4. Мини-приложение через [window.WebApp.initDataUnsafe.start_param](bridge.md#Работа%20с%20данными%20инициализации) обрабатывает стартовые параметры запуска, такие как `payload`
