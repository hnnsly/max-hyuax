...
// Contacts — телефоны участников для сотрудника ответственной УК. Остальным (соседям, чужой УК,
// району) возвращается nil без ошибки: карточку они видят, контакты — нет.
// Участников у заявки десятки, поэтому пользователи читаются по одному.
func (s *Service) Contacts(ctx context.Context, viewer user.User, is *issue.Issue) ([]Contact, error) {
	if !viewer.CanManageIssues(is.ResponsibleOrgID()) {
		return nil, nil
	}
	var out []Contact
